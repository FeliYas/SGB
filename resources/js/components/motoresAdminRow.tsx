...
export default function MotoresAdminRow({ motor }) {
    ...
    const handleUpdate = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        updateForm.post(route('admin.motores.update'), {
            preserveScroll: true,
            onSuccess: () => {
                toast.success('Motor actualizada correctamente');
                setEdit(false);
            },
            onError: (errors) => {
                toast.error('Error al actualizar motor');
                console.log(errors);
            },
        });
    };
    ...
    const deleteMarca = () => {
        if (confirm('¿Estas seguro de eliminar esta motor?')) {
            updateForm.delete(route('admin.motores.destroy'), {
                preserveScroll: true,
                onSuccess: () => {
                    toast.success('Motor eliminada correctamente');
                },
                onError: (errors) => {
                    toast.error('Error al eliminar motor');
                    console.log(errors);
                },
            });
        }
    ...
    return (
        <tr className={`border text-black odd:bg-gray-100 even:bg-white`}>
            <td className="align-middle">{motor?.order}</td>
            <td className="h-[90px] align-middle">{motor?.name}</td>
                    ...
                    >
                        <form onSubmit={handleUpdate} method="POST" className="text-black">
                            <div className="w-[500px] rounded-md bg-white p-4">
                                <h2 className="mb-4 text-2xl font-semibold">Actualizar Motor</h2>
