import MotoresAdminRow from '@/components/motoresAdminRow';
...
export default function MotoresAdmin() {
    const { motores } = usePage().props;
    ...
    const handleSubmit = (e) => {
        ...
        post(route('admin.motores.store'), {
            preserveScroll: true,
            onSuccess: () => {
            ...
            onError: (errors) => {
                toast.error('Error al crear motor');
                console.log(errors);
            },
        });
    };
        ...
        <Dashboard>
            <div className="flex w-full flex-col p-6">
                        ...
                        >
                            <form onSubmit={handleSubmit} method="POST" className="text-black">
                                <div className="w-[500px] rounded-md bg-white p-4">
                                    <h2 className="mb-4 text-2xl font-semibold">Crear Motor</h2>
                ...
                <div className="mx-auto flex w-full flex-col gap-3">
                    <h2 className="border-primary-orange text-primary-orange text-bold w-full border-b-2 text-2xl">Motores</h2>
                    ...
                    <div className="flex w-full justify-center">
                        <table className="w-full border text-left text-sm text-gray-500 rtl:text-right dark:text-gray-400">
                            ...
                            <tbody className="text-center">{motores.data?.map((motor) => <MotoresAdminRow key={motor.id} motor={motor} />)}</tbody>
                        </table>
                    </div>
                            ...
                            <div className="flex items-center">
                                {motores.links.map((link, index) => (
                                    <button
                                        ...
                                        } ${index === 0 ? 'rounded-l-md' : ''} ${index === motores.links.length - 1 ? 'rounded-r-md' : ''}`}
                    ...
                    <div className="mt-2 text-center text-sm text-gray-600">
                        Mostrando {motores.from || 0} a {motores.to || 0} de {motores.total} resultados
                    </div>
                </div>
            </div>
        </Dashboard>
